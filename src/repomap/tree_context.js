...
class TreeContext {
  ...
  addContext() {
    ...
    if (this.loiPad) {
      for (let line of this.showLines) {
        for (let newLine of [line - this.loiPad, line + this.loiPad]) {
          if (
            Array.from(this.scopes[line] || []).find((s) =>
              this.scopes[newLine].has(s)
            )
          ) {
    ...
    if (this.parentContext)
      this.linesOfInterst.forEach((i) => {
        this.addParentScopes(i);
        // console.debug('Show lines: ', this.showLines)
      });
    if (this.childContext)
      this.linesOfInterst.forEach((i) => this.addChildContext(i));
  ...
  addChildContext(line) {
    ...
    let children = [];
    for (let node of this.nodes[line])
