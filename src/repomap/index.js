...
const { filenameToLang } = require("./language_mapper");
const { TreeContext } = require("./tree_context");
...
class RepoMap {
  ...
  getRankedTags(chatFnames, otherFnames = []) {
    ...
    for (let ident of idents) {
      ...
      references.get(ident)?.forEach((referencer) => {
        ...
        for (let definer of definers) {
          ...
          myGraph.addEdge(referencer, definer, {
            weight: counters[referencer],
            ident: ident,
          });
        }
      });
    ...
    myGraph.forEachNode((src) => {
      ...
      let totalWeight = 0;
      myGraph.forEachOutboundEdge(
        src,
        (_, attributes) => (totalWeight += attributes.weight)
      );
      myGraph.forEachOutboundEdge(src, (edge, attributes, source, target) => {
        ...
        rankedDefinitions.set(
          rankedDefKey,
          (rankedDefinitions.get(rankedDefKey) || 0.0) + attributes.rank
        );
      });
    });

    let rankedTags = [];
    const sortedRankedDefinitions = [...rankedDefinitions.entries()].sort(
      (a, b) => b[1] - a[1]
    );
    ...
    const relOtherFnamesWithoutTags = new Set(
      otherFnames.map((fname) => this.getRelFname(fname))
    );
